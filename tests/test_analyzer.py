...
class TestBiasLensAnalyzer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.patcher_analyzer_bias_ml = patch('biaslens.analyzer.BiasLensAnalyzer._analyze_bias_safe') # For ML bias
        ...
        # Default mock return values for analyze method's dependencies
        ...
        self.mock_analyzer_patterns.return_value = {
            ...
            'fake_news': {'detected': False, 'details': {'risk_level': 'low', 'matched_phrases': [], 'is_clickbait': False}},
            'viral_manipulation': {'is_potentially_viral': False, 'engagement_bait_score': 0.1, 'sensationalism_score': 0.1}
        }
        self.mock_analyzer_lw_enhancer.get_lightweight_bias_assessment.return_value = {
        ...
        self.mock_analyzer_overall_assessment.return_value = {} # Output not directly used for final keys
    ...
    def test_analyze_empty_text_core_solution_structure(self):
        ...
        # Assert new "Core Solution" fields are None
    ...
    def test_analyze_core_solution_structure_success(self):
        # Mocks are set up in self.setUp for a generally neutral/low-risk case by default
        # Override specific mocks for this test if needed to show more features
        ...
        self.mock_analyzer_patterns.return_value = {
            ...
            'fake_news': {'detected': True, 'details': {'risk_level': 'high', 'matched_phrases': ['breaking!'], 'is_clickbait': True}},
            'viral_manipulation': {'is_potentially_viral': True, 'engagement_bait_score': 0.8, 'sensationalism_score': 0.7}
        }
        mock_lw_assessment = {"inferred_bias_type": "Anti-PDP political bias", "bias_category": "political", "bias_target": "PDP", "matched_keywords": ["pdp", "bad"]}
        ...
        self.assertEqual(result['explanation'], ['Risky content.'])
        self.assertEqual(result['tip'], 'Verify carefully.')
        ...
        self.assertEqual(tone['primary_tone'], 'anger')
        self.assertTrue(tone['is_emotionally_charged'])
        self.assertEqual(tone['emotional_manipulation_risk'], 'medium')
        self.assertEqual(tone['sentiment_label'], 'negative')
        self.assertEqual(tone['sentiment_confidence'], 0.88)

        # Bias Analysis
        self.assertIn('bias_analysis', result)
        bias = result['bias_analysis']
        self.assertEqual(bias['primary_bias_type'], 'confirmation_bias') # ML takes precedence
        self.assertEqual(bias['bias_strength_label'], 'Strong Bias Detected')
        self.assertEqual(bias['ml_model_confidence'], 0.92)
        self.assertEqual(bias['source_of_primary_bias'], 'ML Model') # Since ML was specific

        # Manipulation Analysis
        self.assertIn('manipulation_analysis', result)
        manipulation = result['manipulation_analysis']
        self.assertTrue(manipulation['is_clickbait']) # True because Nigerian, FakeNews, and Viral all indicated it
        self.assertEqual(manipulation['engagement_bait_score'], 0.8)
        self.assertEqual(manipulation['sensationalism_score'], 0.7)

        # Veracity Signals
        self.assertIn('veracity_signals', result)
        veracity = result['veracity_signals']
        self.assertEqual(veracity['fake_news_risk_level'], 'high')
        self.assertEqual(veracity['matched_suspicious_phrases'], ['breaking!'])
    ...
    def test_bias_analysis_source_of_primary_bias(self):
        ...
        self.mock_analyzer_lw_enhancer.get_lightweight_bias_assessment.return_value = {"inferred_bias_type": "No specific patterns detected"}
        ...
        self.assertEqual(result['bias_analysis']['ml_model_confidence'], 0.8)
        ...
        self.mock_analyzer_lw_enhancer.get_lightweight_bias_assessment.return_value = {"inferred_bias_type": "Anti-Igbo ethnic bias", "bias_category": "ethnic"}
        result = analyze("Text", include_patterns=True)
        ...
        self.assertIsNone(result['bias_analysis']['ml_model_confidence'])

        # Scenario 3: Both ML and Pattern are specific and different (ML should take precedence)
        self.mock_analyzer_bias_ml.return_value = {'flag': True, 'label': 'Biased', 'type_analysis': {'type': 'gender_bias', 'confidence': 0.85}}
        self.mock_analyzer_lw_enhancer.get_lightweight_bias_assessment.return_value = {"inferred_bias_type": "Anti-Igbo ethnic bias", "bias_category": "ethnic"}
        result = analyze("Text", include_patterns=True)
        self.assertEqual(result['bias_analysis']['source_of_primary_bias'], 'ML Model')
        self.assertEqual(result['bias_analysis']['primary_bias_type'], 'gender_bias')
        self.assertEqual(result['bias_analysis']['ml_model_confidence'], 0.85)

        # Scenario 4: Both ML and Pattern are specific and same
        self.mock_analyzer_bias_ml.return_value = {'flag': True, 'label': 'Biased', 'type_analysis': {'type': 'ethnic_bias_igbo', 'confidence': 0.85}} # Hypothetical specific type
        self.mock_analyzer_lw_enhancer.get_lightweight_bias_assessment.return_value = {"inferred_bias_type": "ethnic_bias_igbo", "bias_category": "ethnic"}
        result = analyze("Text", include_patterns=True)
        self.assertEqual(result['bias_analysis']['source_of_primary_bias'], 'ML Model and Pattern Analysis') # Or just "ML Model" depending on exact logic in main code
        self.assertEqual(result['bias_analysis']['primary_bias_type'], 'ethnic_bias_igbo')

        # Scenario 5: No specific bias from either
        self.mock_analyzer_bias_ml.return_value = {'flag': False, 'label': 'Neutral', 'type_analysis': {'type': 'neutral', 'confidence': 0.9}}
        self.mock_analyzer_lw_enhancer.get_lightweight_bias_assessment.return_value = {"inferred_bias_type": "No specific patterns detected"}
        result = analyze("Text", include_patterns=True)
        self.assertEqual(result['bias_analysis']['source_of_primary_bias'], 'ML Model') # As ML classified as neutral
        self.assertEqual(result['bias_analysis']['primary_bias_type'], 'neutral')
    ...
    def test_analyze_detailed_results_core_solution(self):
        ...
        self.assertEqual(sub_analyses['sentiment'], self.mock_analyzer_sentiment.return_value)
        self.assertEqual(sub_analyses['emotion'], self.mock_analyzer_emotion.return_value)
        self.assertEqual(sub_analyses['bias'], self.mock_analyzer_bias_ml.return_value)
        self.assertEqual(sub_analyses['patterns'], self.mock_analyzer_patterns.return_value)
        self.assertEqual(sub_analyses['lightweight_nigerian_bias'], self.mock_analyzer_lw_enhancer.get_lightweight_bias_assessment.return_value)
    ...
    # --- Quick Analyze tests remain largely the same but ensure they still pass ---
